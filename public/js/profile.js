...
function displayProfile(user) {
    ...
    document.getElementById('profileRole').textContent = user.role || 'Artist';
    document.getElementById('profilePicture').src = user.profilePicture || 'images/user2.jpeg';
    
    // Save to localStorage and trigger storage event
    const profileData = {
        name: user.name || 'Default User',
        role: user.role || 'Artist',
        avatar: user.profilePicture || 'images/user2.jpeg'
    };
        ...
        if (file) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (profilePic) {
                        profilePic.src = updatedUser.profilePicture || 'images/default-avatar.png';
                    }
                } else {
    ...
    try {
        ...
        if (response.ok) {
            ...
            localStorage.setItem('userProfile', JSON.stringify({
                ...
                role: updatedUser.role || 'Artist',
                avatar: updatedUser.profilePicture || 'images/user2.jpeg'
            }));
