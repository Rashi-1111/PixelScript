...
const coverUpload = multer({
    storage: createDiskStorage('stories'),
...
function mapPreviewStory(story) {
    ...
    return {
        ...
        chapters: story.chapters.map(chapter => ({
            ...
            previewImage: chapter.panels?.[0]?.imageUrl || projectPanels[0]?.imageUrl || story.coverImage
        }))
    };
}
...
function buildStoryResponse(story, purchases = [], currentUserId = null, currentUserRole = null) {
    ...
    const chapters = storyData.chapters.map(chapter => {
        ...
        return {
            ...
            panels: Array.isArray(chapter.panels) ? chapter.panels : []
        };
    });
...
router.post('/', auth, coverUpload.single('coverImage'), async (req, res) => {
    try {
        ...
        const story = new Story({
            ...
            coverImage: req.file ? buildPublicFileUrl('stories', req.file.filename) : 'images/default-cover.png',
            author: req.user.id,
...
router.put('/:id', auth, coverUpload.single('coverImage'), async (req, res) => {
    try {
        ...
        if (req.file) story.coverImage = buildPublicFileUrl('stories', req.file.filename);
